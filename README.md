...
For the PICO 4 Ultra hybrid teleoperation policy, resume the exact accepted
contract-v9 `model_1499.pt` state through the fail-closed contract-v10 migration
driver. Start with a 100-update canary:
...
scripts/evaluate_microban_teleop_v10_stage.sh --canary <v10-run-name>
scripts/train_microban_teleop_v10.sh resume <v10-run-name>
```

`train` prefixes the requested suffix with a timestamp. Use the full directory
name printed by training as `<v10-run-name>` for evaluation and the next resume.

The active, reproducible migration, staged curriculum, evaluation, recovery and
deployment procedure is in
[`docs/teleop_v10_fixed_lr_recovery.md`](docs/teleop_v10_fixed_lr_recovery.md).
The observation/action details remain in
[`docs/pico_teleop_policy.md`](docs/pico_teleop_policy.md); its v8/v9 history is
