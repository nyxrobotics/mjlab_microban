#!/usr/bin/env bash
# Evaluate one exact contract-v10 stage boundary or interrupted canary.
set -euo pipefail
...
usage() {
    cat <<'EOF'
Usage:
  scripts/evaluate_microban_teleop_v10_stage.sh RUN_NAME
  scripts/evaluate_microban_teleop_v10_stage.sh --canary RUN_NAME

Boundary mode evaluates seeds 42/43/44. Boundaries 10,000 and 15,000 also
require three moving-HMD reports. Only the final 15,000 boundary enforces
performance. Canary mode evaluates the fixed six-scenario yaw-sensitive subset
under seed 42 and publishes a diagnostic hard-safety receipt with no deployment
authority.
EOF
}
...
[[ "${MICROBAN_TELEOP_GATE_FORCE:-0}" =~ ^[01]$ ]] \
    || fail "MICROBAN_TELEOP_GATE_FORCE must be 0 or 1."
...
if (( canary == 1 )); then
    ...
    case "${COMPLETED_ITERATIONS}" in
        ...
        15000)
            ...
            ;;
        *)
            fail "Latest checkpoint has ${COMPLETED_ITERATIONS} completed updates, not a v10 stage boundary. Use --canary for an aligned interrupted checkpoint."
            ;;
    esac
fi

# Fail before GPU work if the checkpoint is not the requested canonical stage.
uv run --locked python - "${CHECKPOINT_PATH}" "${COMPLETED_ITERATIONS}" \
    "${canary}" <<'PY'
import sys
...
print(
    "[PASS] canonical v10 checkpoint preflight: "
...
for seed in "${seeds[@]}"; do
    ...
    evaluator_status=$?
    set -e
    if (( performance_enforced == 1 && canary == 0 )); then
        expected_status=0
    else
        expected_status=3
    fi
    ...
    if (( moving_hmd_required == 1 )); then
        ...
        if (( performance_enforced == 0 )); then
            moving_args+=(--intermediate-hard-safety-only)
        fi
        if [[ -n "${scenarios}" ]]; then
            moving_args+=(--scenarios "${scenarios}")
        fi
        set +e
        uv run --locked python -m mjlab_microban.scripts.evaluate_teleop_checkpoint \
            "${moving_args[@]}"
...
if (( canary == 1 )); then
    uv run --locked python - "${receipt_path}" "${CHECKPOINT_PATH}" \
        "${RUN_NAME}" "${COMPLETED_ITERATIONS}" "${report_paths[0]}" <<'PY'
import sys

from mjlab_microban.scripts.teleop_v10_stage import publish_canary_receipt

receipt = publish_canary_receipt(
    sys.argv[1], sys.argv[2], sys.argv[3], int(sys.argv[4]), sys.argv[5]
)
print(
    "[PASS] v10 interrupted canary (non-deployment): "
    f"{sys.argv[1]} checkpoint={receipt['checkpoint_sha256']}"
)
PY
else
    uv run --locked python - "${receipt_path}" "${CHECKPOINT_PATH}" \
        "${RUN_NAME}" "${COMPLETED_ITERATIONS}" \
        "${#report_paths[@]}" "${report_paths[@]}" \
        "${moving_hmd_report_paths[@]}" <<'PY'
import sys

from mjlab_microban.scripts.teleop_v10_stage import publish_stage_gate
...
print(
    f"[PASS] v10 boundary {sys.argv[4]}, seeds 42/43/44: {sys.argv[1]} "
    f"acceptance_profile={gate['acceptance_profile']}"
)
PY
fi
