...
def validate_deadline_fallback_descendant(
    ...
    if (
        ...
        or iteration != MICROBAN_TELEOP_V12_DEADLINE_CANARY_ITERATION
    ):
...
def validate_deadline_fallback_resume_payload(
    ...
    """Allow training resume from selected v1, never from its finished canary."""
    ...
    if (
        ...
        raise ValueError(
            "Deadline foot canary is complete; post-canary promotion is required"
        )
...
def validate_deadline_fallback_training_request(
    *,
    ...
    if (
        ...
        or common_step_counter != MICROBAN_TELEOP_V12_DEADLINE_FALLBACK_COMMON_STEP
        or num_learning_iterations != 100
        or save_interval != 15_000
    ):
        raise ValueError(
            "Deadline fallback must run only the exact 10000->10100 canary "
            "with save_interval=15000"
        )
...
def validate_deadline_fallback_save_endpoint(
    *, iteration: int, common_step_counter: int, filename: str
    ...
    if (
        ...
        or common_step_counter != MICROBAN_TELEOP_V12_DEADLINE_CANARY_COMMON_STEP
        or filename != f"model_{MICROBAN_TELEOP_V12_DEADLINE_CANARY_ITERATION}.pt"
    ):
        raise RuntimeError(
            "Deadline fallback may save only exact model10099/10100 endpoint"
        )
